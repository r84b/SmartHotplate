...
class WebInterface:
    ...
    async def connect_wifi(self):
        ...
        retries = 0
        while not self.wlan.isconnected() and retries < 30:
            await asyncio.sleep(1)
            retries += 1
            print(f"WiFi retry {retries}/30...")
    ...
    async def start(self):
        interfaces.api.set_context(self.context)
    ...
    async def _serve(self, reader, writer):
        try:
            ...
            if await interfaces.api.handle_requests(path, writer, method, headers, reader):
                return
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if msg.get("type") == "set":
                        self.context.set_target_temp(float(msg["target_temp"]))
                        self.context.set_target_rpm(int(msg["target_rpm"]))
                except Exception as e:
        ...
        while True:
            try:
                if self.websocket_clients:
                    data = {
                        "temp": self.context.read_temp(),
                        "rpm": self.context.get_rpm()
                    }
                    msg = json.dumps(data)
                    frame = create_websocket_frame(msg)
                    dead = []
                    for client in self.websocket_clients:
                        try:
                            ...
                            dead.append(client)
                    for d in dead:
                        if d in self.websocket_clients:
                            self.websocket_clients.remove(d)
            except Exception as e:
...
.value{font-size:3em;margin:1em;}
input{font-size:1em;}
...
<div><input id="target-temp" type="number" step="0.5"> °C
<button onclick="setTarget()">Set Temp</button></div>
<div><input id="target-rpm" type="number" step="50">
<button onclick="setTarget()">Set RPM</button></div>
<script>
